...
public class TimerService implements Serializable {
    ...
    public void insertMetric(Map<String, Object> value) {
        dbm.getCollection(MetricType.TIMER.getCollectionName()).insertOne(new Document(value));
    }
    ...
    public List<TimerDTO> retrieveMetrics(long fromTime, long toTime, String key) {
        ...
        //exec query
        MongoCursor<Document> result = dbm.getCollection(MetricType.TIMER.getCollectionName())
    ...
    public List<TimerDTO> retrieveSumMetrics(long fromTime, long toTime, String key) {
        List<TimerDTO> timers = new ArrayList<>();
        ...
        //exec query
        MongoCursor<Document> result = dbm.getCollection(MetricType.TIMER.getCollectionName()).aggregate(params).iterator();
        
        //convert document result into dto
        while (result.hasNext()) {
            Document statsDoc = result.next();
            timers.add(new TimerDTO(toTime, statsDoc.getInteger("totalCount")));          
        }
